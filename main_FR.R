...
figure.1.data[,Frequency_FR:=as.factor(Frequency_FR)][,Frequency_FR:=reorder(Frequency_FR,c(rep(7,17),rep(6,17),rep(5,17),rep(4,17),rep(3,17),rep(2,17),rep(1,17)))]
...
figure.1 <- plot.column.dais(figure.1.data,
  ...
  scale_fill_manual(values=c("Quelques fois par heure"="#7f3051",
                             ...
                             "Je n’utilise pas ce média"="#3b3b3b",
                             "Je ne suis pas sûr(e)"="grey"))+
  coord_flip() +
  ...
  scale_colour_manual(values=c("Quelques fois par heure"="white",
                             "Quelques fois par jour"="white",
                             "Quelques fois par semaine"="black",
                             "Quelques fois par mois"="black",
                             "Quelques fois par année"="black",
                             "Je n’utilise pas ce média"="white",
                             "Je ne suis pas sûr(e)"="black"))+
...
figure.6.data[,Platform_FR:=as.factor(Platform_FR)][,Platform_FR:=reorder(Platform_FR,rev(c(rep(1,4),
                                                                                        rep(2,4),
                                                                                        rep(3,4),
                                                                                        rep(4,4),
                                                                                        rep(5,4),
...
figure.7.data[,Platform_FR:=str_wrap(Platform_FR, 20)]
figure.7.data[,Platform_FR:=as.factor(Platform_FR)][,Platform_FR:=reorder(Platform_FR,c(rep(1,2),
                                                                                              rep(2,2),
                                                                                              rep(3,2),
                                                                                              rep(4,2),
                                                                                              rep(5,2),
...
figure.8.data[,Platform_FR:=as.factor(Platform_FR)][,Platform_FR:=reorder(Platform_FR,1:6)]
...
figure.9 <- plot.column.dais(figure.9.data,Share,Statement_FR,order.bar = "ascending",group.by=Answer_FR,stacked=TRUE,
  ...
  #ylab(NULL) + scale_y_discrete(breaks = NULL, expand=c(0,0))+
  geom_text(aes(label = label.b,colour = Answer_FR), family="Replica-Light",
...
figure.10 <- plot.column.dais(figure.10.data,Share,Platform_FR,group.by=Trust_FR,stacked=TRUE,
  ...
  geom_text(aes(label = paste0(round(Share, 0), "%"), colour=Trust_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_color_manual(values=c("Haut (7 à 9)"="white","Moyen (4 à 6)"="black","Faible (1 à 3)"="black","Je ne sais pas"="black")) +
...
figure.12.data <- figure.12.data %>%
  ...
  ungroup() %>%
  as.data.table()
figure.12.data[,Platform_FR:=str_wrap(Platform_FR,30)][,Platform_FR:=as.factor(Platform_FR)][,Platform_FR:=reorder(Platform_FR,rep(seq(8,1),4))]
...
figure.14.data[,Statement_FR:=str_wrap(Statement_FR,30)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,rep(seq(6,1),7))]
figure.14.data[,Frequency_FR:=as.factor(Frequency_FR)][,Frequency_FR:=reorder(Frequency_FR,c(rep(7,6),rep(6,6),rep(5,6),rep(4,6),rep(3,6),rep(2,6),rep(1,6)))]
...
figure.14 <- plot.column.dais(figure.14.data,Share,Statement_FR,group.by=Frequency_FR,stacked=TRUE,
  ...
  geom_text(aes(label = paste0(round(Share, 0), "%"),colour=Frequency_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  geom_text(data=figure.14.data.total,aes(label=paste0(round(Share),"%"),x=Statement_FR,y=110),colour="Black",family="Replica-Bold",inherit.aes=FALSE) +
  scale_color_manual(values=c("Quelques fois par jour"="white","Quelques fois par mois"="black","Jamais"="white",
                              "Quelques fois par semaine"="black","Quelques fois par année"="black","Je ne suis pas sûr(e)"="black")) +
  guides(fill=guide_legend(reverse=TRUE, title=NULL),colour="none") +
  theme(axis.title.y = element_blank())+
...
figure.15 <- ggplot(figure.15.data,aes(Year,Share,fill=Frequency_FR)) + dais.base.theme() +
  ...
  geom_text(aes(label = paste0(round(Share, 0), "%"), colour=Frequency_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_colour_manual(values=c("Au moins quelques fois par semaine"="white","Quelques fois par mois"="black","Quelques fois par an"="black")) +
  guides(fill=guide_legend(reverse=TRUE, title=NULL), colour="none") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())+
...
figure.16.data[,Statement_FR:=str_wrap(Statement_FR,30)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,rep(seq(3,1),3))]
...
figure.16 <- plot.column.dais(figure.16.data,Share,Statement_FR,group.by=Answer_FR,stacked=TRUE,
  ...
  geom_text(aes(label = label.b,colour=Answer_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_colour_manual(values=c("Oui"="white","Non"="white","Je ne suis pas sûr(e)"="black"))+
  guides(fill=guide_legend(reverse=TRUE, title=NULL),colour="none") +
  theme(axis.title.y = element_blank())+
  scale_y_continuous(expand=c(0,0),limits=c(0,102),breaks=c(0,25,50,75,100),labels=c("0%","25%","50%","75%","100%"))
...
figure.18.data[,Statement_FR:=str_wrap(Statement_FR,25)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,rep(seq(1,5),2))]
...
figure.20 <- plot.column.dais(figure.20.data,Trust,Point_FR,
  ...
  annotate("text",x=2,y=18,label="Total inefficace: 28%",family="Replica-Bold") +
  annotate("text",x=8,y=19,label="Total efficace: 34%",family="Replica-Bold") +
...
figure.21.data[,Point_FR:=str_wrap(Point_FR,20)]
...
figure.22 <- plot.column.dais(figure.22.data,Share,Statement_FR,group.by=Truth_FR,stacked=TRUE,
  ...
  geom_text(aes(label = label.b,colour=Truth_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_color_manual(values=c("Certainement vrai"="white","Plutôt vrai"="black","Plutôt faux"="black",
                              "Certainement faux"="white","Je ne sais pas"="black"))  +
  guides(fill=guide_legend(reverse=TRUE,title=NULL,ncol=3),colour="none") +
  theme(axis.title.y = element_blank())+
  scale_y_continuous(expand=c(0,0),limits=c(0,101),breaks=c(0,25,50,75,100),labels=c("0%","25%","50%","75%","100%"))
...
figure.23 <- ggplot(figure.23.data,aes(Category_FR,Share,fill=Correct_FR)) + dais.base.theme() +
  ...
  #ylab(NULL) +
  geom_text(aes(label = paste0(round(Share, 0), "%"), colour=Correct_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_colour_manual(values=c("6-8 correct"="white","3-5 correct"="black","0-2 correct"="black")) +
  guides(fill=guide_legend(reverse=TRUE, title=NULL), colour="none") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())+
  scale_y_continuous(expand=c(0,0),limits=c(0,102),breaks=c(0,25,50,75,100),labels=c("0%","25%","50%","75%","100%"))
...
figure.28.data[,Statement_FR:=str_wrap(Statement_FR,50)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,rep(seq(12,1),6))]
...
figure.28 <- plot.column.dais(figure.28.data,Share,Statement_FR,group.by=Support_FR,stacked=TRUE,
  ...
  #ylab(NULL) + scale_y_discrete(breaks = NULL, expand=c(0,0))+
  geom_text(data=figure.28.data[Support_FR %in% c("Fortement favorable",
                                                  "Plutôt favorable",
                                                  "Neutre")],aes(label = paste0(round(Share, 0), "%"),colour=Support_FR), family="Replica-Light",
            position = position_stack(vjust = 0.5)) +
  scale_color_manual(values=c("Fortement favorable"="white","Plutôt favorable"="black","Neutre"="black")) +
  guides(fill=guide_legend(reverse=TRUE, title=NULL),color="none") +
  theme(axis.title.y = element_blank())+
  scale_y_continuous(expand=c(0,0),limits=c(0,101),breaks=c(0,25,50,75,100),labels=c("0%","25%","50%","75%","100%"))
...
figure.29.data[,Statement_FR:=str_wrap(Statement_FR,50)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,c(rep(1,4),
                                                                                                                     rep(2,4),
                                                                                                                     rep(3,8),
                                                                                                                     rep(4,4),
                                                                                                                     rep(5,8),
...
figure.29 <- ggplot(figure.29.data,aes(Year,Share,fill=Support_FR)) + dais.base.theme() +
  ...
  geom_text(aes(label = paste0(round(Share, 0), "%"),colour=Support_FR), family="Replica-Light",size=2.7,
            position = position_stack(vjust = 0.5)) +
  scale_colour_manual(values=c("Fortement favorable"="white","Plutôt favorable"="black")) +
  guides(fill=guide_legend(reverse=TRUE, title=NULL),colour="none") +
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())+
  scale_y_continuous(expand=c(0,0),limits=c(0,102),breaks=c(0,25,50,75,100),labels=c("0%","25%","50%","75%","100%"))
...
figure.31 <- plot.column.dais(figure.31.data,Share,Statement_FR,
  ...
  annotate("text",x=1.5,y=38,label="Favourable total: 61%",family="Replica-Bold") +
  annotate("text",x=4.5,y=19,label="Défavorable total: 20%",family="Replica-Bold") +
  coord_cartesian(clip = 'off') +
...
figure.32.data[,Statement_FR:=str_wrap(Statement_FR,25)][,Statement_FR:=as.factor(Statement_FR)][,Statement_FR:=reorder(Statement_FR,seq(1,4))]
...
export.dais.plot("Exported/French/Figure_28_FR.pdf",figure.28,p.height = 8.0)
export.dais.plot("Exported/French/Figure_28_FR.svg",figure.28,p.height = 8.0,type="svg")
export.dais.plot("Exported/French/Figure_29_FR.pdf",figure.29,p.height = 8.0)
export.dais.plot("Exported/French/Figure_29_FR.svg",figure.29,p.height = 8.0,type="svg")
...
export.dais.plot("Exported/French/Figure_14_FR_short.pdf",figure.14,p.height = 6)
export.dais.plot("Exported/French/Figure_14_FR_short.svg",figure.14,p.height = 6,type="svg")
